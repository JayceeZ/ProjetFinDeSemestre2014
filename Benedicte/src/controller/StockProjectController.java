...
public class StockProjectController {
	...
	public int affichageFiltreAppareil(ArrayList<String> filtre){
		...
		model.Type typeFiltre = null; //Il y a besoin du "model." ‡ cause du conflit de nommage avec quelque chose en JAVA qui existe dÈj‡.
		OS osFiltre = null;
		
		// Permet de voir si la personne a rentrÈ entre 1 et 3 mots clÈs pour le filtre.
		if(filtre.size()>3 || filtre.size()==0)
			return 2;
		
		boolean passage = true;
		
		// Boucle sur les Ètats et met passage a false si l'Ètat entrÈ en premiËre info
		// dans filtre ne correspond pas ‡ un Ètat de la classe Etat.
		...
		// MÍme principe pour cette boucle mais sur les types.
		...
		// MÍme principe pour cette boucle mais sur les OS.
		for(OS os : OS.values()) {
	...
	public int achatAppareil(ArrayList<Integer> achat) {
		if(achat.size()!=2)
			return 2;//nombre d'informations donn√© invalide
