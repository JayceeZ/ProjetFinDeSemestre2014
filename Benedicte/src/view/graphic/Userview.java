...
public class Userview extends JPanel {
	 ...
	 * 
	 * @param parent
	 *            La fen�tre qui contient ce panneau
	 * @param c
	...
	/**
	 * Permet de passer d'un menu � l'autre
	 */
	...
	public void updateInfosUser() {
		...
		infosUser.setText("Connect� en tant que " + emp.getNom() + " ("
	...
	public Map<String, ActionListener> menuGestionnaire() {
		Map<String, ActionListener> m = new HashMap<String, ActionListener>();
		m.put("G�rer les emprunts", new ActionUtilisateur("emprunts"));
		m.put("Liste des emprunteurs", new ActionUtilisateur("userlist"));
		m.put("G�rer le stock", new ActionUtilisateur("stock"));
		m.put("Statistiques", new ActionUtilisateur("stats"));
		return m;
	}

	/**
	 * Lance la proc�dure de nouvel emprunt,<br />
	 ...
	 * Si la creation de l'emprunt est valide, passe � validerDates()
	 */
	...
	private void validerDates() {
		...
		if (!controller.ajouterDateDebutEmprunt(dateDebut)) {
			empruntSelector.setDateText("La date de d�but est incorrecte.");
		} else {
	...
	public void affichageEmprunts(String who) {
		...
		switch(who) {
		case "user":
			//TODO Quand la m�thode recuperer liste emprunts sera cr�e dans controller
			break;
		case "all":
			//TODO Idem
